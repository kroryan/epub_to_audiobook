...
- 2025-10-16: Added Coqui TTS support with improved model selection UI.
- 2025-10-16: **Enhanced Kokoro TTS Integration with Multi-Language Support** - Added comprehensive Kokoro TTS integration with dedicated UI tab, language selector (Spanish, English, French, etc.), voice filtering, and automatic environment configuration. Now supports 67+ voices including Spanish voices like `em_santa`, `ef_dora`, and `em_alex`.
...
- **TTS Provider Selection**: Easy switching between Azure, OpenAI, Edge, Piper, and Coqui TTS with provider-specific options
...
2. **Choose your TTS provider** from the tabs (OpenAI, Azure, Edge, Piper, or Coqui)
3. **Configure provider-specific settings**:
   ...
   - **Coqui**: Select language and model with automatic downloads, or enter custom model path
...
options:
  ...
  --tts {azure,openai,edge,piper,coqui}
                        ...
                        Cognitive Services, openai: OpenAI TTS API, edge: Edge TTS,
                        piper: Piper TTS, coqui: Coqui TTS. When using
...
Coqui TTS provides high-quality, local text-to-speech synthesis using various neural models. Models are automatically downloaded on first use.

1. **Basic conversion using Coqui with default settings**  
   This command will convert an EPUB file to an audiobook using Coqui's default English model.

   ```sh
   python3 main.py "path/to/book.epub" "path/to/output/folder" --tts coqui
   ```

2. **Coqui conversion with specific language and model**  
   Converts an EPUB file to an audiobook using a specific language and model.

   ```sh
   python3 main.py "path/to/book.epub" "path/to/output/folder" --tts coqui --language en --model_name "tts_models/en/ljspeech/tacotron2-DDC_ph"
   ```

3. **Coqui conversion with custom model path**  
   Uses a custom Coqui model file if you have downloaded it manually.

   ```sh
   python3 main.py "path/to/book.epub" "path/to/output/folder" --tts coqui --model_name "/path/to/custom/model.pth"
   ```

4. **Coqui conversion with custom parameters**  
   Adjusts speech speed, noise, and variability for fine-tuned audio quality.

   ```sh
   python3 main.py "path/to/book.epub" "path/to/output/folder" --tts coqui --model_name "tts_models/en/ljspeech/tacotron2-DDC" --coqui_length_scale 0.8 --coqui_noise_scale 0.5 --coqui_noise_w_scale 0.6
   ```
...
**English:**
- **Dedicated Kokoro Tab**: Separate UI tab specifically for Kokoro TTS configuration
- **Language Selector**: Choose from 9 languages including Spanish, English, French, etc.
- **Voice Filtering**: Smart filtering shows relevant voices based on selected language
- **67+ Voices Available**: Access to all Kokoro voices including Spanish ones like `em_santa`, `ef_dora`, `em_alex`
- **Automatic Configuration**: Environment variables set automatically for seamless operation
...
The following Spanish voices are now available through the enhanced UI:

| Voice Name | Description | Descripción |
|------------|-------------|-------------|
| `em_santa` | Male Spanish voice | Voz masculina española |
| `pm_santa` | Alternative male Spanish | Voz masculina española alternativa |
| `am_santa` | Another male Spanish variant | Otra variante masculina española |
| `ef_dora` | Female Spanish voice | Voz femenina española |
| `pf_dora` | Alternative female Spanish | Voz femenina española alternativa |
| `em_alex` | Male Spanish voice | Voz masculina española |
| `pm_alex` | Alternative male Spanish | Voz masculina española alternativa |

### 🌍 Language Codes / Códigos de Idioma

| Code | Language | Idioma |
|------|----------|--------|
| `a` | English (US) | Inglés (EE.UU.) |
| `b` | British English | Inglés Británico |
| `e` | Spanish | Español |
| `f` | French | Francés |
| `h` | Hindi | Hindi |
| `i` | Italian | Italiano |
| `j` | Japanese | Japonés |
| `p` | Portuguese | Portugués |
| `z` | Chinese | Chino |

### 🚀 Quick Start with Spanish Voices / Inicio Rápido con Voces Españolas
...
4. Click "Kokoro" tab
5. Select "Spanish" language
6. Choose voice (e.g., `em_santa`, `ef_dora`)
7. Upload EPUB and click "Start"
...
**English:**
- **Hybrid OpenAI Client**: Automatically detects when to use direct HTTP requests vs OpenAI SDK
- **Language Parameter Support**: Proper handling of Kokoro's language parameter that's not supported by OpenAI SDK
- **Smart Voice Discovery**: Fetches voices dynamically from Kokoro server
- **Error Handling**: Graceful fallbacks when language parameter is not supported
- **Environment Auto-Setup**: Automatically configures `OPENAI_BASE_URL` and `OPENAI_API_KEY`
...
**Main Repository:** [SourceForge](https://sourceforge.net/projects/epub-to-audiobook/)
- Git repository: `https://git.code.sf.net/p/epub-to-audiobook/code`
- SSH clone: `ssh://krory90@git.code.sf.net/p/epub-to-audiobook/code`

**Mirror Repository:** [GitHub](https://github.com/kroryan/epub_to_audiobook_exe)
